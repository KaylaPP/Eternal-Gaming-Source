...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'lets-play':
				dialogue = CoolUtil.coolTextFile(Paths.txt('lets-play/dialog'));
			case 'pause':
				dialogue = CoolUtil.coolTextFile(Paths.txt('pause/pogger'));
			case 'game-over':
				dialogue = CoolUtil.coolTextFile(Paths.txt('game-over/poggin'));
		}
			...
			case 'bglol':
				{
					defaultCamZoom = 0.85;
					curStage = 'bglol';
					var bg:FlxSprite = new FlxSprite(-1125, -395).loadGraphic(Paths.image('mr_gamer/bglol!', 'shared'));
					bg.setGraphicSize(Std.int(bg.width * 0.609));
					bg.antialiasing = true;
		...
		switch (curStage)
		{
			...
			case 'bglol':
				...
				boyfriend.y -= 150;
				gf.x += -259;
				gf.y -= 180; //idk im lazy changing it to -= expect += lol!
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
				...
				#end
				camFollow.setPosition(boyfriend.getMidpoint().x - 300 + offsetX, boyfriend.getMidpoint().y - 120 + offsetY);
